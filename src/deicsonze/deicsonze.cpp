...
void Deicsonze::initCtrls() {
	...
	for(int k=0; k<NBROP; k++) {
		_ctrl[i].name=(std::string(ARSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_AR+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXAR;
		_ctrl[i].name=(std::string(D1RSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_D1R+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXD1R;
		_ctrl[i].name=(std::string(D2RSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_D2R+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXD2R;
		_ctrl[i].name=(std::string(RRSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_RR+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXRR;
		_ctrl[i].name=(std::string(D1LSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_D1L+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXD1L;
		_ctrl[i].name=(std::string(LSSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_LS+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXLS;
		_ctrl[i].name=(std::string(RSSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_RS+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXRS;
		_ctrl[i].name=(std::string(EBSSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_EBS+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXEBS;
		_ctrl[i].name=(std::string(AMESTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_AME+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=1;
		_ctrl[i].name=(std::string(KVSSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_KVS+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXKVS;
		_ctrl[i].name=(std::string(OUTSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_OUT+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXOUT;
		_ctrl[i].name=(std::string("Centi")+std::string(RATIOLONGSTR)+std::to_string(k+1))
			.toLatin1().data();
		_ctrl[i].num=CTRL_RATIO+k*DECAPAR1;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXRATIO*100;
		_ctrl[i].name=(std::string(DETSTR)+std::to_string(k+1)).toLatin1().data();
	...
	_ctrl[i++].max=MAXPL;
	for(int k=0; k<NBROP; k++) {
		_ctrl[i].name=(std::string(FIXSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_FIX+k*DECAPAR2;
		_ctrl[i].min=0;
		_ctrl[i++].max=1;
		_ctrl[i].name=(std::string("Centi")+std::string(FIXRANGESTR)
		               +std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_FIXRANGE+k*DECAPAR2;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXFIXRANGE*100;
		_ctrl[i].name=(std::string(OSWSTR)+std::to_string(k+1)).toLatin1().data();
		_ctrl[i].num=CTRL_OSW+k*DECAPAR2;
		_ctrl[i].min=0;
		_ctrl[i++].max=MAXOSW;
		_ctrl[i].name=(std::string(SHFTSTR)+std::to_string(k+1)).toLatin1().data();
...
void Deicsonze::loadSutulaPresets()
{
	...
	//std::string presetPath(INSTPREFIX);
	//presetPath += "/share/" PACKAGEVERSION "/presets/deicsonze/ARCH_ALIN";

	std::string presetPath("/home/a-lin/sources/svnMusEDev/lmuse/muse/synti/deicsonze/ARCH_ALIN");
...
//   }
	...
	while (!node.isNull()) {
		...
		if (e.tagName() == "deicsOnzeSet") {
			std::string version = e.attribute(std::string("version"));
			if (version == "1.0") {
